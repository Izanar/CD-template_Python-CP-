...
class TestDesignerTaskHistoryAfterActions:
    async def test_create_event_recorded(
        ...
        assert any(item["event"] == TaskEvent.TASK.value and "created" in item["event_info"].lower() for item in data)
    ...
    async def test_update_event_recorded(
        ...
        assert any(item["event"] == TaskEvent.TASK.value and "updated" in item["event_info"].lower() for item in data)
    ...
    async def test_send_event_recorded(
        ...
        assert any(
            item["event"] == TaskEvent.STATUS.value and DesignerTaskStatus.WAITING_TO_ASSIGN.value in item["event_info"]
            for item in data
        )
    ...
    async def test_request_edit_event_recorded(
        ...
        assert any(
            item["event"] == TaskEvent.STATUS.value and DesignerTaskStatus.IN_PROGRESS.value in item["event_info"]
            for item in data
        )
    ...
    async def test_approve_edit_event_recorded(
        ...
        assert any(item["event"] == TaskEvent.EDIT.value and "approved" in item["event_info"] for item in data), (
            "EDIT APPROVED event was not recorded"
        )
    ...
    async def test_approve_task_event_recorded(
        ...
        assert any(
            item["event"] == TaskEvent.STATUS.value and "completed" in item["event_info"].lower() for item in data
        ), f"No STATUS with 'completed'. History: {data}"
